 ...
 * the simulated MCU. Argument hex_ok indicates that the MCU device
 * and frequency are supplied elsewhere, so a HEX file is acceptable,
 * while progname is the current program name for error messages.
 *
                   ...
                   elf_firmware_t * fp, int hex_ok, const char * progname)
{
        ...
        if (suffix && !strcasecmp(suffix, ".hex")) {
                ...
                for (int ci = 0; ci < cnt; ci++) {
                        if (chunk[ci].baseaddr < (1*1024*1024)) {
                                ...
                                printf("Load HEX flash %08x, " "%d\n",
                                       fp->flashbase, fp->flashsize);
                        } else if (chunk[ci].baseaddr >=
                ...
                if (elf_read_firmware(filename, fp) == -1) {
                        fprintf(stderr,
                                "%s: Unable to load firmware from file %s\n",
                                progname, filename);
                        exit(1);
                }
        }
